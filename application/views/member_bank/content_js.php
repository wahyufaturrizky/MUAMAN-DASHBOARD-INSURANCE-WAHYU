...
function datatablepaging() {
     ...
     $.ajax({
          url :"<?php echo site_url();?>memberbank/paging?asuransi="+$('#asuransi').val()+"&search="+$('#contain').val(),
          type:"POST",
          data:{
            },
          success: function(result){
...
function datatableview(i) {
      $('#example1').dataTable({ 
        ...
        //"sScrollXInner": "100%" ,
        "searching" :false, 
        "ajax": "<?php echo site_url()?>memberbank/ajax_data/?asuransi="+$('#asuransi').val()+"&page="+i+"&search="+$('#contain').val()
      });
