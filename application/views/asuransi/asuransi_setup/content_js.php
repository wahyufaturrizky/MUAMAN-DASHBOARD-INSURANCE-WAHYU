...
$('#save').click(function() {
  ...
  if(validation != 'VALID'){
    ...
    $("#modalwarning").modal("show");
       ...
       $.ajax({
          url :"<?php echo site_url();?>registrasi/asuransi_setup/save",
          type:"POST",
          data:{
        		wilayah       :$("#wilayah").val(),
        		pertanggungan :$("#pertanggungan").val(),
            kategoriknd   :$("#kategoriknd").val(),
            rate          :$("#rate").val()
            },
...
function datatablepaging() {
     ...
     $.ajax({
          url :"<?php echo site_url();?>registrasi/asuransi_setup/paging?search="+$('#contain').val(),
          type:"POST",
          data:{
            },
...
function datatableview(i) {
      $('#example1').dataTable({ 
        ...
        "sScrollXInner": "100%" ,
        "searching" :false, 
        "ajax": "<?php echo site_url()?>registrasi/asuransi_setup/ajax_data/?page="+i+"&search="+$('#contain').val()
      });
      ...
      $('#example1 tbody').on('click', '#edit', function () {
        ...
        $("#wilayah").val(data[7]);
        $("#pertanggungan").val(data[8]);
        $("#kategoriknd").val(data[9]);
        $("#rate").val(data[3]);
        $("#id").val(data[6]); 
        $("#modalform").modal("show");
      });
