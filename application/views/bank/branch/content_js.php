...
$('#save').click(function() {
  ...
  if(validation != 'VALID'){
    ...
    $("#modalwarning").modal("show");
       ...
       $.ajax({
          url :"<?php echo site_url();?>registrasi/bank_branch/save",
          type:"POST",
          data:{
        		nama      :$("#nama").val(),
        		alamat    :$("#address").val(),
...
$('#update').click(function() {
  ...
  if(validation != 'VALID'){
    ...
    $("#modalwarning").modal("show");
        ...
        $.ajax({
          url :"<?php echo site_url();?>registrasi/bank_branch/update",
          type:"POST",
          data:{
            nama    :$("#nama").val(),
            alamat  :$("#address").val(),
...
$('#dodelete').click(function() {
   $.ajax({
      url :"<?php echo site_url();?>registrasi/bank_branch/delete",
      type:"POST",
      data:{
      id : $('#deleteid').val(),
      },
...
function datatablepaging() {
   $.ajax({
      url :"<?php echo site_url();?>registrasi/bank_branch/paging?id_parent="+$('#id_parent').val()+"&search="+$('#contain').val(),
      type:"POST",
      data:{
        },
...
function datatableview(i) {
      $('#example1').dataTable({ 
         ...
         "sScrollXInner": "100%" ,
         "searching" :false,    
        "ajax": "<?php echo site_url()?>registrasi/bank_branch/ajax_data/?id_parent="+$('#id_parent').val()+"&page="+i+"&search="+$('#contain').val()
      });
