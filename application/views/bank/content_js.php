...
$('#save').click(function() {
  ...
  if(validation != 'VALID'){
    ...
    $("#modalwarning").modal("show");
       ...
       $.ajax({
          url :"<?php echo site_url();?>registrasi/bank/save",
          type:"POST",
          data:{
        		nama    :$("#nama").val(),
        		alamat  :$("#address").val(),
...
$('#update').click(function() {
  ...
  if(validation != 'VALID'){
    ...
    $("#modalwarning").modal("show");
        ...
        $.ajax({
          url :"<?php echo site_url();?>registrasi/bank/update",
          type:"POST",
          data:{
            nama    :$("#nama").val(),
            alamat  :$("#address").val(),
...
function datatablepaging() {
   $.ajax({
      url :"<?php echo site_url();?>registrasi/bank/paging?search="+$('#contain').val(),
      type:"POST",
      data:{
        },
...
function datatableview(i) {
      $('#example1').dataTable({ 
         ...
         "sScrollXInner": "100%" ,
         "searching" :false,    
        "ajax": "<?php echo site_url()?>registrasi/bank/ajax_data/?page="+i+"&search="+$('#contain').val()
      });
