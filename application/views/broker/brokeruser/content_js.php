...
$('#save').click(function() {
  ...
  if(validation != 'VALID'){
    ...
    $("#modalwarning").modal("show");
       ...
       $.ajax({
          url :"<?php echo site_url();?>registrasi/broker_user/save",
          type:"POST",
          data:{
        		nama      :$("#nama").val(),
        		part_of_id    :$("#part_of_id").val(),
            email     :$("#email").val(),
  ...
  if($('#password').val() != $('#confirm').val()){
     ...
     $("#contentmodalwarning").html("Maaf Ada kesalahan dalam data yang anda input sebagai berikut :<ul>"+validation+"</ul>");
      $("#modalwarning").modal("show");
       ...
       $.ajax({
          url :"<?php echo site_url();?>registrasi/broker_user/save_user",
          type:"POST",
          data:{
            username    :$("#username").val(),
            password    :$("#password").val(),
            id_user     :$("#id_user").val(),
            part_of_id  :$("#brokergroup").val()
            },
          success: function(html){
               $("#form_user")[0].reset();
               $("#modalform").modal("hide");
...
$('#dodelete').click(function() {
   $.ajax({
      url :"<?php echo site_url();?>registrasi/broker_user/delete",
      type:"POST",
      data:{
      id : $('#deleteid').val(),
      },
...
$('#update').click(function() {
  ...
  if(validation != 'VALID'){
    ...
    $("#modalwarning").modal("show");
        ...
        $.ajax({
          url :"<?php echo site_url();?>registrasi/broker_user/update",
          type:"POST",
          data:{
            nama    :$("#nama").val(),
            part_of_id  :$("#part_of_id").val(),
...
function datatablepaging() {
   $.ajax({
      url :"<?php echo site_url();?>registrasi/broker_user/paging?id_broker="+$('#id_broker').val()+"&search="+$('#contain').val(),
      type:"POST",
      data:{
        },
...
function datatableview(i) {
      $('#example1').dataTable({ 
         ...
         "sScrollXInner": "100%" ,
         "searching" :false,    
        "ajax": "<?php echo site_url()?>registrasi/broker_user/ajax_data/?id_broker="+$('#id_broker').val()+"&page="+i+"&search="+$('#contain').val()
      });
...
function isvalid() {
    ...
    if(messageblank!= '' || messageemail != ''){
      result = messageblank+messageemail;
    }
